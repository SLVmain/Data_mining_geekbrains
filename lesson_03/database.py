...
class Database:
    ...
    def get_or_create_сomments(self, session, model, **data):
        db_data = session.query(model).filter(model.body == data["body"]).first()
        if not db_data:
            db_data = model(**data)
        return db_data
    ...
    def create_post(self, data):
        ...
        comments = map(
            lambda com_data: self.get_or_create_сomments(session, models.Comment, **com_data),
            data['comments']
        )
        author = self.get_or_create(session, models.Author, **data['author'])
        #comment = self.get_or_create_сomments(session, models.Comment, **data['comments'])   , comment=comment
