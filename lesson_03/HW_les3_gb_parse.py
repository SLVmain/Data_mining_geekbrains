...
class GbParse:
    ...
    def post_parse(self, url, soup):
        ...
        post_datetime = soup.find("time", attrs={"class": "text-md text-muted m-r-md"}).text.strip()
        post_datetime = post_datetime.split(" ")
        post_datetime = datetime.datetime(int(post_datetime[2]), months_dict[post_datetime[1][:3]], int(post_datetime[0]))
